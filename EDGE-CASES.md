...
6. **Monitoring Class/Instance Methods:** 
   ...
   1. **Syntax Rules:** You can monitor methods, but they must **not** be defined using shorthand syntax (`myMethod() {}`). It will crash the parser. Only arrow function syntax (`myMethod = () => {}`) and function expression syntax (`myMethod = function() {}`) will work. If the package tried to handle shorthand syntax, it would force the internal code generator to parse complex edge cases (getters, setters, constructors), which is highly error-prone. 
 
   2. **Preserving the `this` Context:** The correct way to safely monitor an instance method while preserving its `this` context is to use the dedicated `bind` property when setting up the monitored function. See this [example](https://github.com/The-BigMan-tech/fn-monitor/blob/master/examples/monitoring-methods.ts).
