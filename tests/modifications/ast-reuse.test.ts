...
describe('AST Mutation Persistence', () => {
    ...
    it('[Async] should parse the function into an AST once and reuse it, causing mutations to persist across calls',async () => {
        let modifiedOp = false;
        ...
        const monitoredFn = monitor({//this is intentionally not lifted to the top because its doing a stateful operation and it should not affect other tests
            ...
            inspector:(visit) => {
                ...
                visit.is('BinaryExpression', (event) => {
                    event.node.operator = "-"; // Mutate the AST node from '+' to '-'
                    modifiedOp = true;
                });
            }
        });
        ...
        // First call: The inspector mutates the AST. 
        expect(await monitoredFn(5, 1)).toBe(4);// The function executes as `5 - 1`, resulting in 4.


        // Second call: The inspector skips mutation, BUT the AST is already changed.
        expect(modifiedOp).toBe(true);//verify that the operator is modified first.
        expect(await monitoredFn(4, 3)).toBe(1);//The function executes as `4 - 3`, resulting in 1 (proving persistence).
    });
